...
{
public:
	static std::vector<float> Make(int width, int height, float maxRadius = 200.0f, 
		std::pair<float, float> center = { 0.0f, 0.0f }, int nVertices = 50)
	{
		...
		const float dTheta = 2 * PI / (float)nVertices;
