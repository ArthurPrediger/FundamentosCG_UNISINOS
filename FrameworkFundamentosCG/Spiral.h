...
{
public:
	static std::vector<float> Make(int width, int height, float maxRadius = 150.0f, int nLaps = 3, 
		std::pair<float, float> center = { 0.0f, 0.0f }, int nVertices = 500)
	{
		...
		for (int i = 0; i < nVertices; i++, radius += dRadius)
		{
			...
			float x = (cos(angle) * radius + center.first) / halfWidth;
			float y = (sin(angle) * radius + center.second) / halfHeight;
